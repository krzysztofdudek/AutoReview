...
export async function withRetry(fn, { attempts = 3, initialMs = 500, factor = 2, jitterMs = 200, shouldRetry = retryable } = {}) {
  let lastErr;
  for (let i = 0; i < attempts; i++) {
    try { return await fn(); }
      ...
      const delay = initialMs * Math.pow(factor, i) + Math.floor(Math.random() * jitterMs);
