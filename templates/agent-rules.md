...
Only invoke `autoreview:review` with `--sha <commit>` when the user explicitly wants a fresh re-review (rules changed, history log missing, debugging a verdict). `review` costs tokens; `history` doesn't.

Both support `HEAD`, `HEAD~1`, tags, branches, full SHAs.

## Review vs. precommit
- **Pre-commit hook** (soft-fail by default) runs on `git commit`. Stops you only when `enforcement.precommit: hard`. Wraps the same `validate.mjs` script.
- **`autoreview:review` skill** (hard by default) is the explicit manual review path for the agent.
...
## Creating a rule
Use the `autoreview:create-rule` skill. All 7 steps, even if the user tells you to skip — the wizard guards rule quality.
